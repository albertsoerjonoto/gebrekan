...
undangan untuk sophia. next.js + neon postgres + vercel.
...
Sophia bisa balik/lanjut bebas; state di-persist ke `localStorage`. Setiap perubahan di-push ke `/api/track` (debounced 250 ms, dengan fallback `sendBeacon` ketika halaman ditutup) supaya lo bisa lihat dia pindah-pindah pilihan.
...
Sudah dilakukan (branch: `claude/sophia-invitation-gebrekan-KkSRw`). Merge ke `main` atau deploy langsung dari branch ini — Vercel OK dengan keduanya.
